...
#define PARAM_PERSISTED_AT \
    @"pa"  // Persistent Storage Created At. A timestamp created when the app was
           // first launched and the persistent storage was created. Stores as
           // seconds from Unix epoch. (int)
#define PARAM_SEQUENCE_NUMBER \
    @"seq"  // Sequence number - an increasing count for each blob, stored in the
...
// PARAM_DATA_TYPE
#define PARAM_APP_KEY @"au"               // TopAnalytics Application ID
...
#define PARAM_NEW_FLOW_EVENTS \
    @"nw"  // Events and screens encountered during this session that have NOT been staged for
           // upload.
#define PARAM_OLD_FLOW_EVENTS \
    @"od"  // Events and screens encountered during this session that HAVE been staged for upload.

/************************
 * Partner attributions *
 ***********************/
#define FB_ATTRIBUTION @"fb_attrib_first"  // Facebook attribution cookie
#define FB_ATTRIBUTION_TIME \
    @"fb_attrib_first_date"  // Time original attribution cookie was collected
#define FB_ATTRIBUTION_CURRENT @"fb_attrib_current"  // Facebook attribution cookie
#define FB_ATTRIBUTION_CURRENT_TIME \
    @"fb_attrib_current_date"           // Time original attribution cookie was collected
 ...
 * Custom Parameters *
 ***********************/
#define DISTINCT_DAYS @"dd"            // Every different day user opened the app
#define PARAM_SUMMED_VALUE_NAME @"sv"  // Summed customer value for all events, such as revenue
